...
def build():
    ...
    add_table(
        ...
        [
            ...
            ("当前成熟度", "交互原型 v0.6；Windows 的单行启动、本地识别、结构检查与浏览器试跑均有通过收据，Linux／macOS 尚未实测。当前无统一口径反应能量、催化性能结论或催化剂发现。"),
        ],
        [2.7, 15.6],
        6.65,
    )
    ...
    add_table(
        ...
        [
            ...
            ("OC20（Chanussot et al., 2021）／Materials Project API", "初始／弛豫结构、能量与力／最低能体相晶体", "OC20 非完整反应网络；体相结构不是催化界面"),
            ("Crossref REST API／OpenAlex Works API", "DOI、题名、作者、日期、落地页与开放状态", "只作身份核验和跳转，不给反应能或几何"),
            ("alphaXiv MCP／arXiv API", "认证后 AI 阅读或原文抽取／规范元数据回退", "alphaXiv 默认是 AI 中间报告；fullText=true 才请求原文；无认证 401"),
        ],
        [4.3, 7.2, 6.8],
        5.85,
    )
    ...
    add_p(doc, "环境技术通过：① DOI 经 Crossref／OpenAlex 核验；alphaXiv 只有认证且 fullText=true 才计入原文读取，否则用 arXiv API 回退；② 本地无数值验证器通过反应物／产物、能量 null、固定 50／25／15／10 排序、mp-19306／support-only、日志结构检查，且不生成催化数值；③ 人工终审链接、公式、字段和结论边界。三关全过且反馈改变下一行动。", 7.0, bold_lead="环境技术通过：", after=0.7)

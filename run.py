...
def quick_check() -> dict[str, Any]:
    ...
    try:
        ...
        normalized = result.get("normalized_problem", {})
        stoichiometry = result.get("basis", {}).get("matrix", {})
        if result.get("source") != "local_exact_kernel":
            raise RuntimeError("check did not use the local exact kernel")
        if normalized.get("input_balance") != "invalid":
            raise RuntimeError("unbalanced demo input was not detected")
        if stoichiometry.get("check") != [0, 0, 0]:
            raise RuntimeError("element-conservation check did not return A*nu = 0")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Scope: local structural audit; no catalyst simulation or unconditional ranking.", flush=True)
